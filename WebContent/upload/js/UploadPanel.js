...
UploadPanel = Ext.extend(Ext.Panel, {
	...
	resizeProgressBar : function() {
		this.progressBar.setWidth(this.el.getWidth() - 8);
	},
	startUpload : function() {
		if (this.swfupload) {
			this.swfupload.uploadStopped = false;
			//var post_params = this.swfupload.settings.post_params;
			//post_params.dataList=g_getGridJsonData(Ext.getCmp("fileGridId"));
	...
	closeWin : function() {
//		var store = Ext.getCmp('UploadGrid').getStore();
//		store.reload();
		//Ext.getCmp('UploadGrid').getView().refresh();
		//document.getElementById('upload_time').value='';
		Ext.getCmp('upload_win').hide();
	},
	...
	updateProgressInfo : function() {
		if(this.progressInfo.bytesUploaded>this.progressInfo.bytesTotal){
			this.progressInfo.bytesUploaded=this.progressInfo.bytesTotal;
		}
	...
	formatState : function(_v, cellmeta, record) {
		...
		switch (_v) {
			case SWFUpload.FILE_STATUS.QUEUED :
				returnValue = '<div recid="' + record.id + '" id="fileId_' + record.data.fileId
						+ '" class="ux-cell-icon-delete"/>';
				break;
	...
	onUploadSuccess : function(file, serverData) {
		...
		if (Ext.util.JSON.decode(serverData).success) {
			var record = thiz.fileList.getStore().getById(Ext.getDom('fileId_' + file.id).getAttribute("recid"));			
			record.set('fileState', file.filestatus);
			record.commit();
			thiz.successCall(record);
		}
