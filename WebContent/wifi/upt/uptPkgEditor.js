...
com.wifi.UptPkgEditor = Ext.extend(Ext.Panel,{
	...
	initComponent: function(){
					...
					new com.wifi.UptPkgForm({
					 ...
					 new com.wifi.LimitGrid({
						 ...
						 tbar:this.isDisabled? null:[
	        		       ...
	        		       {xtype:"button",iconCls:'delete',text:"删除区域",handler:function(){Ext.getCmp("limitGridId").deleteRows()}}
	        	         ]
					}),
			        	...
			        	buttons:[	
									 {
									    ...
									    handler : function() {
									    	...
									    	if(!top.g_edGridFieldValid1(limitGrid,"更新包适用区域")) return false;
									    	...
									    	var sucFun=function(){
									    		if((Ext.getCmp("uptIdEd").getValue()).length==0){
										    		baseFrm.reset();
										    		limitGrid.store.removeAll();
									    		}else{
									    			Ext.getCmp("mainViewportId").cardRedirect(false,0);
									    		}
										    };	
										    top.postJsonData(G_ROOT_PATH+'/uptPkgWeb.cll?method=save',postData,sucFun);
									    }
									},{
								    	  xtype:"button",
								    	  text:"返 回",
		...
		com.wifi.UptPkgEditor.superclass.initComponent.call(this);
	},
