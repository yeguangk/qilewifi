...
(function() {
	var idSeed = 0, ua = navigator.userAgent.toLowerCase(), check = function(r) {
		return r.test(ua);
			...
			&& check(/msie 7/), isIE8 = isIE && check(/msie 8/), isGecko = !isWebKit
				...
				function() {
					...
					return {
						...
						flatten : function(arr) {
							...
							function rFlatten(a) {
								Ext.each(a, function(v) {
									Ext.isArray(v) ? rFlatten(v) : worker
											.push(v);
								});
								return worker;
							}
						...
						median : function(arr, sorter, medCalc) {
							...
							if (arr.length) {
								if (arr.length == 1) {
									...
									if (remainder == 0) {
										ret = medCalc ? medCalc(arr[middle],
												arr[middle + 1])
						...
						mode : function(arr) {
							var collect = {}, mode = arr[0];
							Ext.each(arr, function(v) {
								collect[v] == undefined ? collect[v] = 0
										: collect[v]++;
						...
						zip : function() {
							...
							}), arrs = parts[0], fn = parts[1][0], len = Ext
									.max(Ext.pluck(arrs, "length")), ret = [];
							for ( var i = 0; i < len; i++) {
								ret[i] = [];
								if (fn) {
						...
						useShims : ((E.isIE && E.isIE6) || (E.isMac
								&& E.isGecko && !E.isGecko3)),
						type : function(o) {
...
(function() {
	...
	(function() {
		...
		Ext.lib.Dom = {
			...
			getDocumentHeight : function() {
				return MAX(!isCSS1 ? doc.body.scrollHeight
						: doc.documentElement.scrollHeight, this
						.getViewportHeight());
			},
			getDocumentWidth : function() {
				return MAX(!isCSS1 ? doc.body.scrollWidth
						: doc.documentElement.scrollWidth, this
						.getViewportWidth());
			},
			getViewportHeight : function() {
				return Ext.isIE ? (Ext.isStrict ? doc.documentElement.clientHeight
						: doc.body.clientHeight)
						: self.innerHeight;
			},
			getViewportWidth : function() {
				return !Ext.isStrict && !Ext.isOpera ? doc.body.clientWidth
						: Ext.isIE ? doc.documentElement.clientWidth
								: self.innerWidth;
			},
			getY : function(el) {
				...
				if (el != bd) {
					if (el.getBoundingClientRect) {
						...
						while (p) {
							...
							if (Ext.isGecko) {
								y += bt = PARSEINT(pe
										.getStyle("borderTopWidth"), 10) || 0;
								x += bl = PARSEINT(pe
										.getStyle("borderLeftWidth"), 10) || 0;
								if (p != el
	...
	Ext.lib.Event = function() {
		var loadComplete = false, listeners = [], unloadListeners = [], retryCount = 0, onAvailStack = [], _interval, locked = false, win = window, doc = document, POLL_RETRYS = 200, POLL_INTERVAL = 20, EL = 0, TYPE = 1, FN = 2, WFN = 3, OBJ = 3, ADJ_SCOPE = 4, doAdd = function() {
			var ret;
		...
		function checkRelatedTarget(e) {
			var related = e.relatedTarget, isXulEl = Object.prototype.toString
					.apply(related) == '[object XULElement]';
			if (!related)
				return false;
			return (!isXulEl && related != this && this.tag != 'document' && !elContains(
					this, related));
		}
				...
				Ext.each(onAvailStack, function(v, i, a) {
					if (v && (element = doc.getElementById(v.id))) {
						if (!v.checkReady || loadComplete
							...
							element = v.override ? (v.override === true ? v.obj
									: v.override) : element;
		...
		var pub = {
			...
			removeListener : function(el, eventName, fn) {
				...
				if (!fn) {
					...
					Ext.each(unloadListeners,
							function(v, i, a) {
								if (v && v[0] == el && v[1] == evantName
										&& v[2] == fn) {
			...
			resolveTextNode : function(node) {
				return Ext.isSafari && node && 3 == node.nodeType ? node.parentNode
						: node;
			},
			...
			getListeners : function(el, eventName) {
				var me = this, results = [], searchLists = [ listeners,
						unloadListeners ];
				if (eventName) {
			...
			_unload : function(e) {
				...
				Ext.each(unloadListeners, function(v) {
					if (v) {
						scope = v[ADJ_SCOPE] ? (v[ADJ_SCOPE] === true ? v[OBJ]
								: v[ADJ_SCOPE]) : win;
		...
		var pub = {
			request : function(method, uri, cb, data, options) {
				if (options) {
					...
					if (xmlData || jsonData) {
						initHeader('Content-Type', xmlData ? 'text/xml'
								: 'application/json');
																...
																if (opt.selected) {
																	data += String
																			.format(
																					"{0}={1}&",
																					encoder(name),
																					(opt.hasAttribute ? opt
																							.hasAttribute('value')
																							: opt
																									.getAttribute('value') !== null) ? opt.value
																							: opt.text);
																}
															});
										} else if (!/file|undefined|reset|button/i
												.test(type)) {
	...
	Ext.lib.Region.prototype = {
		...
		intersect : function(region) {
			var me = this, t = Math.max(me.top, region.top), r = Math.min(
					me.right, region.right), b = Math.min(me.bottom,
					region.bottom), l = Math.max(me.left, region.left);
			if (b >= t && r >= l) {
				return new Ext.lib.Region(t, r, b, l);
			}
		},
		union : function(region) {
			var me = this, t = Math.min(me.top, region.top), r = Math.max(
					me.right, region.right), b = Math.max(me.bottom,
					region.bottom), l = Math.min(me.left, region.left);
			return new Ext.lib.Region(t, r, b, l);
		},
	...
	Ext.lib.Region.getRegion = function(el) {
		var p = Ext.lib.Dom.getXY(el), t = p[1], r = p[0] + el.offsetWidth, b = p[1]
				+ el.offsetHeight, l = p[0];
		return new Ext.lib.Region(t, r, b, l);
	};
	...
	(function() {
		var EXTLIB = Ext.lib, noNegativesRE = /width|height|opacity|padding/i, defaultUnitRE = /width|height|top$|bottom$|left$|right$/i, offsetUnitRE = /\d+(em|%|en|ex|pt|in|cm|mm|pc)$/i;
		EXTLIB.Anim = {
			...
			run : function(el, args, duration, easing, cb, scope, type) {
				...
				var anim = new type(el, args, duration, EXTLIB.Easing[easing]
						|| easing);
		...
		EXTLIB.AnimBase.prototype = {
			...
			getAttr : function(attr) {
				var flyEl = fly(this.el), val = flyEl.getStyle(attr), getter;
				if (val !== 'auto' && !offsetUnitRE.test(val)) {
					return parseFloat(val);
				}
				getter = flyEl['get' + attr.charAt(0).toUpperCase()
						+ attr.substr(1)];
				return getter ? getter.call(flyEl) : 0;
			},
			setRunAttr : function(attr) {
				var me = this, isEmpty = Ext.isEmpty, a = me.attrs[attr], unit = a.unit, by = a.by, from = a.from, to = a.to, ra = (me.runAttrs[attr] = {}), start, end;
				if (isEmpty(to) && isEmpty(by))
					return false;
				start = !isEmpty(from) ? from : me.getAttr(attr);
				end = !isEmpty(to) ? to : [];
				if (!isEmpty(by)) {
					if (Ext.isArray(start)) {
						Ext.each(start, function(v, i, s) {
							end[i] = v + by[i];
						});
					} else {
						end = start + by;
				...
				ra.start = start;
				ra.end = end;
				ra.unit = !isEmpty(unit) ? unit
						: (defaultUnitRE.test(attr) ? 'px' : '');
			},
			init : function(el, attribute, duration, method) {
				...
				me.animate = function(callback, scope) {
					...
					me.curFrame = 0;
					me.totalFrames = (me.useSec) ? Math.ceil(anmgr.fps
							* duration) : duration;
					if (!me.isAnimated)
						anmgr.registerElement(me);
				};
				...
				function onTween() {
					...
					for ( var attr in me.runAttrs) {
						...
						me.setAttr(attr, me.doMethod(attr, ra.start, ra.end),
								ra.unit);
					}
					actualFrames++;
				}
		...
		EXTLIB.AnimMgr = function() {
			var thread = new Ext.util.TaskRunner(), pub;
			function correctFrame(tween) {
				var frames = tween.totalFrames, frame = tween.curFrame, duration = tween.dur, expected = (frame
						* duration * 1000 / frames), elapsed = (new Date() - tween.startTime), tweak = 0;
		...
		(function() {
			function bezier(points, t) {
				var len = points.length, tmp = points.slice(0), C = (1 - t), i, j;
				for (j = 1; j < len; ++j) {
					for (i = 0; i < len - j; ++i) {
						var ti = tmp[i];
			...
			EXTLIB.Motion = function(el, attrs, duration, method) {
				if (el) {
					EXTLIB.Motion.superclass.constructor.call(this, el, attrs,
							duration, method);
				}
			};
			...
			Ext
					.apply(
							...
							{
								setAttr : function(attr, val, unit) {
									var setAttr = superclass.setAttr, me = this;
								...
								getAttr : function(attr) {
									var getAttr = superclass.getAttr, me = this;
									return pointsRE.test(attr) ? [
											getAttr.call(me, 'left'),
											getAttr.call(me, 'top') ] : getAttr
											.call(me, attr);
								},
								doMethod : function(attr, start, end) {
									var me = this;
									return pointsRE.test(attr) ? bezier(
											me.runAttrs[attr], me.method(
													me.curFrame, 0, 100,
													me.totalFrames) / 100)
											: superclass.doMethod.call(me,
													attr, start, end);
								},
								setRunAttr : function(attr) {
									var me = this;
									if (pointsRE.test(attr)) {
										var el = me.el, attrs = me.attrs, points = attrs.points, control = points.control
												|| [], runAttrs = me.runAttrs, getXY = EXTLIB.Dom.getXY, from = attrs.points.from
												|| getXY(el), start;
										function translateValues(val, start, to) {
											var pageXY = to ? getXY(me.el) : [
													0, 0 ];
											return val ? [
													(val[0] || 0) - pageXY[0]
															+ start[0],
										...
										runAttrs[attr] = [ start = me
												.getAttr('points') ]
												.concat(control);
										runAttrs[attr]
												.push(translateValues(points.to
														|| points.by || null,
														start,
														!Ext.isEmpty(points.to)));
									} else {
	...
	(function() {
		...
		Ext.apply(EXTLIB.Easing, {
			...
			easeBothStrong : function(t, b, c, d) {
				return ((t /= d / 2) < 1) ? c / 2 * t * t * t * t + b : -c / 2
						* ((t -= 2) * t * t * t - 2) + b;
			},
			elasticIn : function(t, b, c, d, a, p) {
				...
				return -(a * pow(2, 10 * (t -= 1)) * sin((t * d - s) * (2 * pi)
						/ p))
						+ b;
			},
			...
			bounceBoth : function(t, b, c, d) {
				return (t < d / 2) ? EXTLIB.Easing.bounceIn(t * 2, 0, c, d)
	...
	(function() {
		...
		var superclass = EXTLIB.ColorAnim.superclass, colorRE = /color$/i, transparentRE = /^transparent|rgba\(0, 0, 0, 0\)$/;
		function parseColor(s) {
			...
			if (s.length == 3) {
				...
				c = s.replace("#", "").match(/./g);
				c = [ pi(c[0] + c[0], 16), pi(c[1] + c[1], 16),
						pi(c[2] + c[2], 16) ];
			} else {
				c = s.replace("#", "").match(/./g);
				c = [ pi(c[0] + c[1], 16), pi(c[2] + c[3], 16),
						pi(c[4] + c[5], 16) ];
			}
			return c;
		}
		Ext
				.apply(
						EXTLIB.ColorAnim.prototype,
						{
							getAttr : function(attr) {
								var me = this, el = me.el, val;
								if (colorRE.test(attr)) {
									while (el
											&& transparentRE.test(val = fly(el)
													.getStyle(attr))) {
										el = el.parentNode;
										val = "fff";
									}
							...
							doMethod : function(attr, start, end) {
								...
								if (colorRE.test(attr)) {
									...
									Ext.each(start, function(v, i) {
										val[i] = superclass.doMethod.call(me,
												attr, v, end[i]);
									});
									val = 'rgb(' + floor(val[0]) + ','
											+ floor(val[1]) + ','
									...
									val = superclass.doMethod.call(me, attr,
											start, end);
								}
								return val;
							},
							setRunAttr : function(attr) {
								...
								superclass.setRunAttr.call(me, attr);
								if (colorRE.test(attr)) {
									var attribute = me.attrs[attr], ra = me.runAttrs[attr], start = parseColor(ra.start), end = parseColor(ra.end);
									if (isEmpty(attribute.to)
											&& !isEmpty(attribute.by)) {
	...
	(function() {
		...
		var Y = Ext.lib, superclass = EXTLIB.Scroll.superclass, SCROLL = 'scroll';
		Ext
				.apply(
						EXTLIB.Scroll.prototype,
						{
							toString : function() {
								var el = this.el;
							...
							doMethod : function(attr, start, end) {
								...
								if (attr == SCROLL) {
									val = [
											me.method(curFrame, start[0],
													end[0] - start[0],
													totalFrames),
											me.method(curFrame, start[1],
													end[1] - start[1],
													totalFrames) ];
								} else {
									val = superclass.doMethod.call(me, attr,
